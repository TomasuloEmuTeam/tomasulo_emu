...
@app.route("/setAllMem", methods=["POST"])
def setAllMem():
...
def getAllMem():
    ...
    [val_0, val_1, ..., val_4095]
    """
    return jsonify(data=mem.getAll())
...
def step():
    ...
    def update(arg):
        ...
        for sta in stations:
            stations[sta]['fn'].update(name, value)

    def reset(name):
        for sta in stations:
            stations[sta]['fn'].reset(name)
    ...
    cycle += 1
    return jsonify({'data': [exeDone(), inst_num]})
